...
const WorkSpaceComponent: FC<WorkSpaceComponentProps> = ({
  ...
  const onSelect = (ws: IObject | null) => {
    clearErrors("parentOrganization");
    setValue("parentOrganization", ws);
    setValue("parentOrganizationId", ws?.id);
    onInstitutionChange && onInstitutionChange(ws);
    setSearchOpen(false);
  };

  const values = getValues(["parentOrganization", "parentOrganizationId"]);
      ...
      <Controller
        ...
        render={({ field, formState: { errors } }) => (
          <Input
            ...
            isError={!!errors?.parentOrganization}
            errorMessage={errors?.parentOrganization?.message as string}
          />
        )}
      />
